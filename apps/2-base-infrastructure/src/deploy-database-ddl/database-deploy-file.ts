...
export class DatabaseDeployFile {
    ...
    public async checkFile() {
        ...
        if ( this.deployRecs.length === 0 ) {
            console.log (`No records found in file ${this.fileName} on bucket ${this.bucketName}`)
            throw new Error (`No records found in file ${this.fileName} on bucket ${this.bucketName}`)
        }
    ...
    private async readDatabaseDeployFile<T>() {
        ...
        try {
            ...
            const dataDeploys = lines.map((line,idx) => {
                ...
                if ( deploy.length !== 9 ) {
                    console.log (`Error reading line ${idx} from file ${this.fileName} from bucket ${this.bucketName}`)
                    this.fileErrors++;
                } else if (deploy[8].trimEnd() !== "") {
                    if ( this.mode !== "" && this.mode !== deploy[8].trimEnd()) {
                        console.log (`Different modes in file found. Previous (${this.mode}) current (${deploy[8].trimEnd()})`)
                        this.fileErrors++;
                    }
                ...
                return {
                    ...
                    instRowCounts: deploy[3].trimEnd() === "" ? null : deploy[3].trimEnd().split(',').map((item) => parseInt(item)),
                    rollbackDesc: deploy[4].trimEnd(),
                    rollbackFilename: deploy[5].trimEnd(),
                    rollbackRowCounts: deploy[6].trimEnd() === "" ? null : deploy[6].trimEnd().split(',').map((item) => parseInt(item)),
            ...
            if (this.fileErrors > 0) {
                throw new Error (`Error in file ${this.fileName} on bucket ${this.bucketName}`)
            }

            return dataDeploys as T[];
            
        } catch (err) {
            console.log (`File may not exist on bucket. File ${this.fileName} from bucket ${this.bucketName}.`)
    ...
    private async checkAllsFileExists( ) {
        ...
        this.deployRecs.forEach( (fileDetails) => {
          ...
          if (fileDetails.instFilename === '' && fileDetails.rollbackFilename === '') {
            console.log (`${fileDetails.deployId} - Have no install or uninstall file.`)
            filesExist = false;
          }
    
          // Check the install file exists on the bucket
          if ( fileDetails.instFilename !== "" && !this.filesInBucket.includes(fileDetails.instFilename) ) {
            console.log (`${fileDetails.deployId} - ${fileDetails.instFilename} does not exist.}`)
            filesExist = false;
          }
    
          // Check the uninstall file exists on the bucket
          if ( fileDetails.rollbackFilename !== "" && !this.filesInBucket.includes(fileDetails.rollbackFilename) ) {
            console.log (`${fileDetails.deployId} - ${fileDetails.rollbackFilename} does not exist.}`)
            filesExist = false;
          }

          // If data filename is set then check it exists
          if ( fileDetails.dataFilename !== "" && !this.filesInBucket.includes(fileDetails.dataFilename) ) {
            console.log (`${fileDetails.deployId} - ${fileDetails.dataFilename} does not exist.}`)
            filesExist = false;
          }
        });
