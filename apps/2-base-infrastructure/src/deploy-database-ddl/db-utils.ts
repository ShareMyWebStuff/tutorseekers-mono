...
export class DbConnection {
    ...
    public async query<T extends RowDataPacket[] | ResultSetHeader[] | RowDataPacket[][] | ProcedureCallPacket>(sql: string) {
        // console.log ('Execute ...')
        if (!this.connection) {
    ...
    public async execute<T extends RowDataPacket[] | ResultSetHeader[] | RowDataPacket[][] | ProcedureCallPacket>(sql: string) {
        // console.log ('Execute ...')
        if (!this.connection) {
            // console.log ('Connection ...')
            this.connection = await this.connectToDB();
    ...
    public async beginTran () {
        try {
            ...
            if ( typeof err  === 'object' && err !== null && !Array.isArray(err) && 'message' in err && typeof err.message === 'string') {
                throw new ErrorHandler(500, err['message'], `${this.__moduleName}__BeginTran`)
            } else {
                throw new ErrorHandler(500, 'Database error: starting transaction', `${this.__moduleName}__BeginTran`)
            }
        }
    }
    ...
    public async commitTran () {
        try {
            ...
            if ( typeof err  === 'object' && err !== null && !Array.isArray(err) && 'message' in err && typeof err.message === 'string') {
                throw new ErrorHandler(500, err['message'], `${this.__moduleName}__CommitTran`)
            } else {
                throw new ErrorHandler(500, 'Database error: commiting transaction', `${this.__moduleName}__CommitTran`)
            }
        }
    }
    ...
    public async rollbackTran () {
        try {
            ...
            if ( typeof err  === 'object' && err !== null && !Array.isArray(err) && 'message' in err && typeof err.message === 'string') {
                throw new ErrorHandler(500, err['message'], `${this.__moduleName}__RollbackTran`)
            } else {
                throw new ErrorHandler(500, 'Database error: rolling back transaction', `${this.__moduleName}__RollbackTran`)
            }
        }
    }
