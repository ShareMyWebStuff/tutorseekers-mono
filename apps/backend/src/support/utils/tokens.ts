...
export const handleAuthCookies = async (
  ...
  const cookies =
    event.headers.cookie === undefined
      ? parseCookie(event.headers.Cookie!)
      : parseCookie(event.headers.cookie);
  if (
    !cookies ||
    ...
    console.log(
      "** handleAuthCookies - Cookies are missing from the request to the backend.",
    );
    console.log(!cookies);
  ...
  try {
    const decoded = verifyJWTToken(cookies["accessToken"], accessSecret);
    ...
    console.log(
      "** handleAuthCookies - Error setting th access token. Lets see if refresh token is good.",
    );
    res.statusCode = 201;
  }
  ...
  if (res.statusCode === 201) {
    try {
      const decoded = verifyJWTToken(cookies["refreshToken"], refreshSecret);
      if (typeof decoded === "string") {
        console.log(
          "** handleAuthCookies - Error decoding the refresh cookie.",
        );
        ...
        console.log(
          "** handleAuthCookies - Creating new refresh and access tokens.",
        );

        res.newAccessToken = createToken(
          {
            userId: decoded.userId,
            accountType: decoded.accountType,
            access: decoded.access,
        ...
        res.newRefreshToken = createToken(
          {
            userId: decoded.userId,
            accountType: decoded.accountType,
            access: decoded.access,
    ...
    try {
      let userTokens = await retrieveUserRefreshTokenById(res.decoded.userId);
      if (userTokens.rows !== 1) {
        console.log(
          "** handleAuthCookies - No rows found for the user on the refresh cookie table.",
        );
      ...
      let foundToken = userTokens.userRefreshToken.find(
        (tk: { userId: number; refreshToken: string }) =>
          tk.refreshToken === cookies["refreshToken"],
      );
      if (foundToken === undefined) {
        console.log(
          "** handleAuthCookies - The refresh cookie does not exist for this user.",
        );
      ...
      console.log(
        "** handleAuthCookies - ERROR retrieving refresh token from database.",
      );
    ...
    try {
      let userTokens = await createUserRefreshToken(
        res.decoded.userId,
        res.newRefreshToken,
      );
      if (userTokens.statusCode !== 201) {
        console.log(
          "** handleAuthCookies - The new refresh cookie has been saved.",
        );
      ...
      console.log(
        "** handleAuthCookies - ERROR saving the new refresh cookie.",
      );
